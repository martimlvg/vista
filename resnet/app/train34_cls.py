...
class TrainData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        msk2 = np.zeros_like(lbl_msk1)  # damaged (minor + major merged)
        msk3 = np.zeros_like(lbl_msk1)  # destroyed
        msk1[lbl_msk1 == 1] = 255
        msk2[(lbl_msk1 == 2) | (lbl_msk1 == 3)] = 255
        msk3[lbl_msk1 == 4] = 255
        ...
        for i in range(try_cnt):
            ...
            _sc = msk2[y0:y0+crop_size, x0:x0+crop_size].sum() * 5 + msk3[y0:y0+crop_size, x0:x0+crop_size].sum() * 2 + msk1[y0:y0+crop_size, x0:x0+crop_size].sum()
            if _sc > bst_sc:
                bst_sc = _sc
                bst_x0 = x0
                bst_y0 = y0
        ...
        if random.random() > 0.983:
            ...
            shape = img.shape[:2]
            alpha = random.uniform(0.25, 1.2) * shape[0]
            sigma = 0.2 * shape[0]
            ...
            img = np.stack([map_coordinates(img[...,c], coords, order=1) for c in range(img.shape[2])], axis=-1).astype('uint8')
        ...
        if random.random() > 0.983:
            ...
            x, y = np.meshgrid(np.arange(shape[1]), np.arange(shape[0]))
            coords = [np.clip(y+dy, 0, shape[0]-1), np.clip(x+dx, 0, shape[1]-1)]
            img2 = np.stack([map_coordinates(img2[...,c], coords, order=1) for c in range(img2.shape[2])], axis=-1).astype('uint8')
...
class ValData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        _loc_path = path.join(loc_folder, fn.split('/')[-1].replace('_pre_disaster.png', '_pre_disaster_part1.png'))
        _loc_img = cv2.imread(_loc_path, cv2.IMREAD_UNCHANGED)
        msk_loc = _loc_img > (0.3*255) if _loc_img is not None else np.zeros((1024, 1024), dtype=bool)
...
def validate(model, data_loader, seg_loss):
    dices0 = []
    ...
    with torch.no_grad():
        for i, sample in enumerate(tqdm(data_loader)):
            ...
            val_loss = 0.05 * loss0 + 0.2 * loss1 + 0.8 * loss2 + 0.5 * loss3
    ...
    for c in range(3):
        precision[c] = tp[c] / (tp[c] + fp[c] + 1e-6)
    ...
    sc = 0.3 * d0 + 0.7 * f1
    print("Val Score: {:.4f}, Dice: {:.4f}, Precision: {:.4f}, F1: {:.4f}, F1_0(no-dmg): {:.4f}, F1_1(damaged): {:.4f}, F1_2(destroyed): {:.4f}, P_0: {:.4f}, P_1: {:.4f}, P_2: {:.4f}".format(
        sc, d0, macro_precision, f1, f1_sc[0], f1_sc[1], f1_sc[2], precision[0], precision[1], precision[2]))
    return sc, {
        'score': sc, 'dice': d0, 'f1': f1,
        'val_loss': val_losses.avg,
        'f1_0': f1_sc[0], 'f1_1': f1_sc[1], 'f1_2': f1_sc[2],
        'precision': macro_precision,
        'p_0': precision[0], 'p_1': precision[1], 'p_2': precision[2]
    }


"""def evaluate_val(data_val, best_score, model, snapshot_name, current_epoch):
    model = model.eval()
    d, metrics = validate(model, data_loader=data_val)
    ...
    print("score: {:.4f}\tscore_best: {:.4f}".format(d, best_score))
    return best_score, metrics""" # using challenge metrics 

def evaluate_val(data_val, best_score, model, snapshot_name, current_epoch, seg_loss):
    model = model.eval()
    d, metrics = validate(model, data_loader=data_val, seg_loss=seg_loss)

    checkpoint_score = metrics['precision']
    ...
    print("val_score: {:.4f}\tbest_precision: {:.4f}".format(d, best_score))
    return best_score, metrics
...
def train_epoch(current_epoch, seg_loss, ce_loss, model, optimizer, scheduler, train_data_loader):
    ...
    for i, sample in enumerate(iterator):
        ...
        loss = 0.05 * loss0 + 0.2 * loss1 + 0.8 * loss2 + 0.5 * loss3
...
if __name__ == '__main__':
    ...
    for fn in tqdm(all_files):
        ...
        fl[0] = (msk1 is not None) and (1 in msk1)
        fl[1] = (msk1 is not None) and (2 in msk1 or 3 in msk1)
        fl[2] = (msk1 is not None) and (4 in msk1)
    ...
    for epoch in range(20):
        ...
        if epoch % 2 == 0:
            ...
            best_score, _ = evaluate_val(val_data_loader, best_score, model, snapshot_name, epoch, seg_loss)
